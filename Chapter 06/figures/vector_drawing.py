...
class Plane():
    def __init__(self, bottom_left, top_right, name=None):
        self.fig = plt.figure(figsize=(top_right[0]-bottom_left[0], top_right[1]-bottom_left[1]))
...
class Space():
    ...
    def plot_function(self,f):
        X,Y = np.meshgrid(np.linspace(self.xmin,self.xmax,100), np.linspace(self.ymin,self.ymax,100))
        self.ax.plot_surface(X, Y, f(X,Y))
        return self
